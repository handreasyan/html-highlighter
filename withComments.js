...
const changeData = (selectedColor) => {
  selectedValues.forEach((select) => {
    ...
    const item1 = document.getElementById(select.id)

    item1.innerHTML = item1.innerHTML.replace(select.obshi, str)
  })

  selectedValues = []

  /* const str = skizb + getSpanContainer(selectedColor, mejtex) + verj
   const item1 = document.getElementById(changeableId1)

   item1.innerHTML = item1.innerHTML.replace(obshi, str)*/
...
const onSelectionchange = (e) => {
  ...
  if (selection.anchorNode.data === selection.focusNode.data) {
    ...
    const f = selection.anchorNode?.data?.substring(0, Math.min(selection.anchorOffset, selection.focusOffset))
    const l = selection.anchorNode?.data?.substring(Math.max(selection.anchorOffset, selection.focusOffset))
    ...
    // skizb - datark , mejtex 0ic anchorofset, verj anchorofsetic length
    // skizv 0 ic focusoffset , mejtex focusoffsetic length , verj datark

    // hakarak qashelu depq  ==============================
    if (selection.toString().length - (selection.anchorNode.length - selection.anchorOffset) < 0) {
      console.log('haklarak')
      selectedValues.push({
        id: findId(selection.anchorNode),
        obshi: selection.anchorNode?.data,
        skizb: '',
