...
lriv normal ashxatuma , uxxaki erb vor linumen nerdrvac noder voronq vor highlight aneluc kanchvuma
 removeBetweenNodesHighlight functiony , inqy current highlight arvaci idnerova sksum man gal u childeri vrayic jnjel
 colory , bayc ejy refreshic heto el inqy chi kara kancvhi qani vor current highlight chka ,

 dra hamar entadrum em karelia ` vercnel root.innerHTML , u ciklov ira mej stugel ` ete kan nerdrvac span_ner voronq
 unenq nuyn BG_n nerdrvacnerin jnjel uxxaki , bayc dra masin hajord dasin ))
...
const saveHighlightData = (id, replacer, value) => {
  const data = JSON.parse(localStorage.getItem("HIGHLIGHTERS") || "[]");
  localStorage.setItem(
    "HIGHLIGHTERS",
    JSON.stringify([...data, { id, replacer, value }])
  );
};
...
const changeData = (selectedColor) => {
  ...
  selectedValues.forEach((select, i) => {
    if (isHasBetweenHighlightedNodes) {
      ...
      if (i === 0) {
        ...
        saveHighlightData(select.id, select.obshi, str);
      } else {
      ...
      const str =
        select.skizb +
        getSpanContainer(selectedColor, select.mejtex) +
        select.verj;
      const item = document.getElementById(select.id);
  ...
  if (selectedValues.length > 1) {
    findBetweenNodes(
      document.getElementById(selectedValues[0].id),
      selectedValues[1].id
    );
    ...
    betweenNodes.forEach((node) => {
      ...
      if (item.localName === "ol" || item.localName === "ul") {
        item.childNodes.forEach((node) => {
          const newStr = getSpanContainer(selectedColor, node.innerHTML);
          node.innerHTML = newStr;
          saveHighlightData(node.id, node.innerText, newStr);
        });
        return;
      }
      ...
      saveHighlightData(
        node.id,
        item.innerText,
        getSpanContainer(selectedColor, item.innerHTML)
      );
    });
  }
...
const onSelectionchange = (e) => {
  ...
  if (selection.anchorNode.data === selection.focusNode.data) {
    ...
    selectedValues.push({
      ...
      skizb: selection.anchorNode?.data?.substring(
        0,
        Math.min(selection.anchorOffset, selection.focusOffset)
      ),
      mejtex: selection.anchorNode?.data?.substring(
        selection.anchorOffset,
        selection.focusOffset
      ),
      verj: selection.anchorNode?.data?.substring(
  ...
  .map((str, index) => {
    ...
    if (!str.includes("id=")) {
      tags.forEach((tag, i) => {
        ...
        const emptyTag = `${tag}> </${tag.substring(1)}>`;
        if (str === emptyTag) {
          str = str.replace(emptyTag, undefined);
          return;
        }
...
const HIGHLIGHTERS = JSON.parse(localStorage.getItem("HIGHLIGHTERS") || "[]");
HIGHLIGHTERS.forEach((item) => {
  const elem = document.getElementById(item.id);
  elem.innerHTML = elem.innerHTML.replace(item.replacer, item.value);
});

document.getElementById("remove").onclick = () => {
  localStorage.setItem("HIGHLIGHTERS", "[]");
  root.innerHTML = joined.join("");
};
